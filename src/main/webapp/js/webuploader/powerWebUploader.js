(function ($, window) {
    var applicationPath = window.appPath === "" ? "" : window.appPath || "../..";
    function SuiJiNum() {
    ...
    function initWebUpload(item, options) {
        ...
        var webuploaderoptions = $.extend({
                ...
                // swf文件路径
                swf: applicationPath + '/js/webuploader/0.1.5/Uploader.swf',
                // 文件接收服务端。
        ...
        if (opts.auto) {
            ...
            uploader.on('fileQueued', function (file) {//队列事件
                $list.append('<div id="' + $(item)[0].id + file.id + '" class="item">' +
                    '<span class="webuploadinfo">' + file.name + '</span>&nbsp;&nbsp;&nbsp;' +
                    '<span class="webuploadstate"><small>等待上传...</small></span>&nbsp;&nbsp;&nbsp;' +
                    '<span class="webuploadDelbtn"><span class="fa fa-close (alias) fa-lg" style="color: red"></span></span><br />' +
                    '</div>');
            });
        }
    ...
    $.fn.powerWebUpload = function (options) {
        ...
        if (typeof WebUploader == 'undefined') {
            var casspath = applicationPath + "/css/webuploader/0.1.5/webuploader.css";
            $("<link>").attr({ rel: "stylesheet", type: "text/css", href: casspath }).appendTo("head");
            var jspath = applicationPath + "/js/webuploader/0.1.5/webuploader.nolog.min.js";
