...
class UUIDTimer {
    ...
    public void getTimestamp(byte uuidData[]) {
        ...
        if(systime < mLastSystemTimestamp) {
            log.warn("System time going backwards! (got value " + systime + ", last " + mLastSystemTimestamp);
        ...
        if(systime <= mLastUsedTimestamp) {
            if((long)mClockCounter < 10000L) {
                ...
                log.warn("Timestamp over-run: need to reinitialize random sequence");
                initCounters(mRnd);
                if(actDiff >= 100L) {
    ...
    throws IOException {
        ...
        if(old != null) {
            try {
                ...
                log.warn("Failed to deactivate the old synchronizer: " + ioe);
            }
        }
        mSync = sync;
        if(sync != null) {
    ...
    private static final void slowDown(long startTime, long actDiff) {
        ...
        log.warn("Need to wait for " + delay + " milliseconds; virtual clock advanced too far in the future");
    ...
    static
    {
        log = LogFactory.getLog(UUIDTimer.class);
    }
}
