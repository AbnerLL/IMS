...
//        throw exception;
    ...
    public long readStamp() {
        ...
        try {
            ...
            log.error("Failed to read file size: " + ioe);
            return 0L;
        }
        mWeirdSize = size != 22;
        if(size == 0) {
            log.warn("Missing or empty file, can not read timestamp value");
            return 0L;
        }
        if(size > 100) {
            size = 100;
        ...
        try {
            ...
            log.error("Failed to read " + size + " bytes: " + ie);
            return 0L;
        }
        ...
        if(result < 0L) {
            log.error("Malformed timestamp file contents: " + err);
            return 0L;
        } else {
    ...
    throws IOException {
        if(stamp <= mLastTimestamp) {
            if(stamp == mLastTimestamp) {
                log.warn("Trying to re-write existing timestamp (" + stamp + ")");
                return;
            } else {
    ...
    protected static void doDeactivate(File f, RandomAccessFile raf, FileLock lock) {
        if(lock != null) {
            try {
                ...
                log.warn("Failed to release lock (for file '" + f + "'): " + t);
            }
        }
        if(raf != null) {
            try {
                ...
                log.warn("Failed to close file '" + f + "':" + t);
            }
        }
    }
    ...
    static
    {
        log = LogFactory.getLog(LockedFile.class);
    }
}
