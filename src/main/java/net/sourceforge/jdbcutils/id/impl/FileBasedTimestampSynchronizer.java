...
//        throw exception;
    ...
    throws IOException {
        ...
        if(result <= 0L) {
            log.warn("Could not determine safe timer starting point: assuming current system time is acceptable");
        } else {
            long now = System.currentTimeMillis();
            long diff = now - result;
            if(now + mInterval < result) {
                log.warn("Safe timestamp read is " + (result - now) + " milliseconds in future, and is greater than the inteval (" + mInterval + ")");
            }
        }
        return result;
    }
    ...
    static
    {
        log = LogFactory.getLog(FileBasedTimestampSynchronizer.class);
    }
}
